<template>
    <div>
        <heading class="mb-6">Config Edit Tool</heading>

        <card
            class="bg-90 flex flex-col items-center justify-center"
            style="min-height: 300px; padding: 30px 0 30px 0;"
        >
            <svg
                class="spin fill-80 mb-6"
                width="69"
                height="72"
                viewBox="0 0 23 24"
                xmlns="http://www.w3.org/2000/svg"
            >
                <path
                    d="M20.12 20.455A12.184 12.184 0 0 1 11.5 24a12.18 12.18 0 0 1-9.333-4.319c4.772 3.933 11.88 3.687 16.36-.738a7.571 7.571 0 0 0 0-10.8c-3.018-2.982-7.912-2.982-10.931 0a3.245 3.245 0 0 0 0 4.628 3.342 3.342 0 0 0 4.685 0 1.114 1.114 0 0 1 1.561 0 1.082 1.082 0 0 1 0 1.543 5.57 5.57 0 0 1-7.808 0 5.408 5.408 0 0 1 0-7.714c3.881-3.834 10.174-3.834 14.055 0a9.734 9.734 0 0 1 .03 13.855zM4.472 5.057a7.571 7.571 0 0 0 0 10.8c3.018 2.982 7.912 2.982 10.931 0a3.245 3.245 0 0 0 0-4.628 3.342 3.342 0 0 0-4.685 0 1.114 1.114 0 0 1-1.561 0 1.082 1.082 0 0 1 0-1.543 5.57 5.57 0 0 1 7.808 0 5.408 5.408 0 0 1 0 7.714c-3.881 3.834-10.174 3.834-14.055 0a9.734 9.734 0 0 1-.015-13.87C5.096 1.35 8.138 0 11.5 0c3.75 0 7.105 1.68 9.333 4.319C16.06.386 8.953.632 4.473 5.057z"
                    fill-rule="evenodd"
                />
            </svg>

            <h1 class="text-white text-4xl text-90 font-light mb-6">
                Настройки VK
            </h1>
            <div class="column-container">
                <div class="column">
                    <div class="column-item" v-for="(value, key, index) in config" v-if="index % 2 === 0">
                        <label :for="key" :key="key" class="config text-white">
                            {{ key | splitWords }}
                        </label>
                        <input
                            type="text"
                            :name="key"
                            :id="key"
                            v-model="config[key]"
                            @keydown.enter.prevent.exact="saveConfig"
                        >
                    </div>
                </div>
                <div class="column">
                    <div class="column-item" v-for="(value, key, index) in config" v-if="index % 2 !== 0">
                        <label :for="key" :key="key" class="config text-white">
                            {{ key | splitWords }}
                        </label>
                        <input
                            type="text"
                            :name="key"
                            :id="key"
                            v-model="config[key]"
                            @keydown.enter.prevent.exact="saveConfig"
                        >
                    </div>
                </div>
            </div>
            <div class="button-container">
                <button @click="saveConfig" name="save_config" type="submit">Отправить</button>
            </div>
        </card>
    </div>
</template>

<script>
export default {
    metaInfo() {
        return {
          title: 'ConfigEditTool'
        }
    },
    data() {
        return {
            config: [],
        }
    },
    filters: {
        splitWords: function(value) {
            // Проверяем, содержит ли значение символы _ или -
            if (value.includes('_')) {
                // Разделяем строку по символу _
                return value.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ');
            } else if (value.includes('-')) {
                // Разделяем строку по символу -
                return value.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ');
            } else {
                // Возвращаем значение без изменений
                return value.replace(/([a-z])([A-Z])/g, '$1 $2').split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1).toLowerCase()).join(' ');
            }
        }
    },
    mounted() {
        this.getConfig();
    },
    methods: {
        getConfig() {
            axios.get('/nova-vendor/config-edit-tool/get').then((response) => {
                this.config = response.data;
            })
        },

        async saveConfig() {
            await axios.post('/nova-vendor/config-edit-tool/save',
                this.config
            ).then(response => {
                this.getConfig();
            })
            .catch(error => {
                this.getConfig();
            });
        }
    },
    setup() {
        const formData = new FormData();

        return {
            formData
        }
    }
}
</script>

<style>
/* Scoped Styles */
    .column-container {
        display: inline-flex;
    }

    .column {
        flex: 1;
        padding-right: 10px;
        box-sizing: border-box;
    }

    .column-item {
        margin-bottom: 10px;
    }

    .config {
        display: flex;
        align-items: center;
        margin-bottom: 5px;
    }

    .column-container input {
        padding: 5px;
        border: 1px solid #ccc;
        margin-bottom: 5px;
    }

    .button-container {
        margin-top: 10px;
    }

    button {
        background-color: #fff;
        border: none;
        padding: 10px 20px;
    }

    button:hover {
        background-color: var(--30);
    }

    button:active {
        background-color: var(--50);
    }
</style>
